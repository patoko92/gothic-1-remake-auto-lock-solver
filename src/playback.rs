...
const BAR_SWITCH_DELAY_MS: u64 = 80;

/// Play a sequence of moves as WASD keyboard input.
/// Checks `playing` flag before each keystroke — stops early if set to false.
///
/// Mapping:
///   W = next bar (bar index +1)
///   S = previous bar (bar index -1)
///   A = Left  (+1 position)
///   D = Right (-1 position)
///
/// We always start navigation from bar 0.
pub fn play_moves(moves: &[Move], playing: &AtomicBool) {
    ...
    for mv in moves {
        ...
        let target_bar = mv.bar;

        // Navigate to target bar via W/S
        while current_bar < target_bar {
            if !playing.load(Ordering::SeqCst) {
                break;
            }
        ...
        thread::sleep(Duration::from_millis(BAR_SWITCH_DELAY_MS));
